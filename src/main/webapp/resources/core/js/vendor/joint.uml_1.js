...
joint.uml.ClassModel = function()
{
	...
	this.imports = [];
	this.type = "",
	this.options = "", //extend or implement class
	this.attributes = [],
	this.methods = []
}
...
joint.uml.Generator = 
{
	langages : ["java", "objC", "php", "swift"],//java langages to generate 
	primitives : ["int", "long", "float", "double", "char", "string", "bool", "boolean"],
	objCEquivalent : {"Integer" : "NSInteger", "Long" : "NSNumber*", "Float" : "NSNumber*", "Double" : "NSNumber*", "String" : "NSString*", "Date" : "NSDate*", "Numeric" : "NSNumber*", "Bool" : "BOOL", "Void" : "void"},
	swiftEquivalent : {"Integer" : "Int", "Long" : "Int32", "Date" : "NSDate", "Numeric" : "Double", "Boolean" : "Bool", "Void" : ""},
	...
	/**
	 * Generate model sources for all langage enabled
	 **/
	generate : function(graph) 
	{
		var zip = new JSZip();
		zip.file("diagram.txt", JSON.stringify(graph.toJSON()));//save diagram as json (backup)
		var classes = graph.getElements();
		for (var i = 0; i < classes.length; i++) 
		{
			var classe = classes[i];
			for (var j = 0; j < this.langages.length; j++) 
			{
				var langage = this.langages[j]; 
				var folder = zip.folder(langage);
				console.log(langage +" class "+ classe);
				switch(langage)
				{
					case "java":
						folder.file(classe.get("name")+".java", this.generateJava(graph, classe));	
					break;
					case "php":
						folder.file(classe.get("name")+".php", this.generatePhp(graph, classe));	
					break;
					case "swift":
						folder.file(classe.get("name")+".swift", this.generateSwift(graph, classe));	
					break;
					case "objC":
						folder.file(classe.get("name")+".h", this.generateObjCh(graph, classe));
						...
						{
							folder.file(classe.get("name")+".m", this.generateObjCm(graph, classe));	
						}
	...
	**/
	generateJava : function(graph, model) 
	{
			...
			if(model.get("id") == link.get("source").id)
			{
				switch (link.get("type")) 
				{
					case "uml.Generalization":
						imports[imports.length] = graph.getCell(link.get("target").id).get("name");
						optionsExtend = "extends "+graph.getCell(link.get("target").id).get("name");			
					break;
					case "uml.Implementation":
						imports[imports.length] = graph.getCell(link.get("target").id).get("name");
						optionsInterface[optionsInterface.length] = graph.getCell(link.get("target").id).get("name");
					break;
				}
			}
		}
		...
		for (var i = 0; i < attributes.length; i++) 
		{
			...
			attributeParts[0] = attributeParts[0].replace("-", "").replace("+", "").replace("#", "").trim();
			var visibility = "public";
			if(isPrivate) visibility = "private";
			if(isProtected) visibility = "protected";
		...
		for (var i = 0; i < methods.length; i++) 
		{
			...
			if(methodParts == null)
			{
				if(console)console.log("ERROR : method "+methods[i].trim()+" not match visibility name(params):return so it's not exported");
				continue;
			}
			...
			for (var k = 0; k < params.length; k++) 
			{
				...
				methodParams[methodParams.length] = parts.reverse().join(" ").trim();
			}
		...
		for (var j = 0; j < links.length; j++) 
		{
			...
			if(model.get("id") == link.get("source").id)
			{
				switch (link.get("type")) 
				{
					case "uml.Generalization":
						imports[imports.length] = graph.getCell(link.get("target").id).get("name");
						optionsExtend = ": "+graph.getCell(link.get("target").id).get("name");			
					break;
					case "uml.Implementation":
						imports[imports.length] = graph.getCell(link.get("target").id).get("name");
						optionsInterface[optionsInterface.length] = graph.getCell(link.get("target").id).get("name");
		...
		for (var i = 0; i < methods.length; i++) 
		{
			...
			if(params != "")
			{
				for (var k = 0; k < params.length; k++) 
				{
					...
					{
						methodParams[methodParams.length] = "and"+this.capitalize(parts[0].trim())+":"+parts.reverse().join(" ").trim();	
					}
			...
			{
				methodObj.returnType = this.objCEquivalent[methodParts[4].trim()];	
			}
	...
	generateSwift : function(graph, model) 
	{
		...
		for (var i = 0; i < attributes.length; i++) 
		{
			...
			if(isArray)
			{
				...
				{
					attrObj.type = "Array<"+this.swiftEquivalent[attributeParts[1]]+">";	
				}
				else 
				{
			...
			else 
			{
				...
				{
					attrObj.type = this.swiftEquivalent[attributeParts[1].trim()];
				}
				else 
				{
					attrObj.type = attributeParts[1].trim();
		...
		{
			constructor[constructor.length] = obj.attributes[i].name+" : "+obj.attributes[i].type;	
		}
		...
		for (var i = 0; i < methods.length; i++) 
		{
			...
			if(!methodParts[4] || methodParts[4].toLowerCase().trim() == "void")
			{
				methodParts[4] = "";
			}
			else if (methodParts[4].indexOf("[]") != -1) 
			{
				...
				{
					methodParts[4] = "-> Array<"+this.swiftEquivalent[methodParts[4]]+">";	
				}
				else 
				{
			...
			else 
			{
				...
				{
					methodParts[4] = "-> "+this.swiftEquivalent[methodParts[4].trim()];
				}
				else 
				{
			...
			if(params != "")
			{
				for (var k = 0; k < params.length; k++) 
				{
					...
					methodParams[methodParams.length] = parts.join(" : ").trim();
				}
			}
			methodObj.visibility = visibility;
			methodObj.name = methodParts[2].trim();
