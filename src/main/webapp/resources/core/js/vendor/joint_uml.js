 ...
 * single mechanism. 3. Use the same single mechanism to support "private" and
 * "user" data. 4. _Never_ expose "private" data to user code (TODO: Drop _data,
 * _removeData) 5. Avoid exposing implementation details on user objects (eg.
 * expando properties) 6. Provide a clear path for implementation upgrade to
 * WeakMap in 2014
 */
						...
						if ( callback ) {
							...
							if ( type === "abort" ) {
								...
								complete(
									...
									xhr.statusText,
									// Support: IE9
									// #11426: When requesting binary data, IE9
...
/**
 * @license Lo-Dash 2.2.1 (Custom Build) <http://lodash.com/> Build: `lodash
...
;(function() {
  ...
  function runInContext(context) {
	 ...
	 * `createCallback`, `curry`, `debounce`, `defaults`, `defer`, `delay`,
	 * `difference`, `filter`, `flatten`, `forEach`, `forEachRight`, `forIn`,
	 * `forInRight`, `forOwn`, `forOwnRight`, `functions`, `groupBy`, `indexBy`,
	 * `initial`, `intersection`, `invert`, `invoke`, `keys`, `map`, `max`,
	 * `memoize`, `merge`, `min`, `object`, `omit`, `once`, `pairs`, `partial`,
	 * `partialRight`, `pick`, `pluck`, `pull`, `push`, `range`, `reject`,
	 * `remove`, `rest`, `reverse`, `shuffle`, `slice`, `sort`, `sortBy`,
	 * `splice`, `tap`, `throttle`, `times`, `toArray`, `transform`, `union`,
	 * `uniq`, `unshift`, `unzip`, `values`, `where`, `without`, `wrap`, and
	 * `zip`
	 * 
	 ...
	 * @example
	 * 
	 * var wrapped = _([1, 2, 3]); // returns an unwrapped value
	 * wrapped.reduce(function(sum, num) { return sum + num; }); // => 6 //
	 * returns a wrapped value var squares = wrapped.map(function(num) { return
	 * num * num; });
	 * 
	 ...
	 * @param {boolean}
	 *            [deep=false] Specify a deep clone.
	 * @param {Function}
	 ...
	 *            level.
	 * @param {boolean}
	 *            [isArgArrays=false] A flag to restrict flattening to arrays
	 ...
	 * @example
	 * 
	 * _.assign({ 'name': 'moe' }, { 'age': 40 }); // => { 'name': 'moe', 'age':
	 * 40 }
	 * 
	 * var defaults = _.partialRight(_.assign, function(a, b) { return typeof a ==
	 * 'undefined' ? b : a; });
	 * 
	 * var food = { 'name': 'apple' }; defaults(food, { 'name': 'banana',
	 * 'type': 'fruit' }); // => { 'name': 'apple', 'type': 'fruit' }
	 */
	 ...
	 * be cloned, otherwise they will be assigned by reference. If a callback is
	 ...
	 * @example
	 * 
	 * var stooges = [ { 'name': 'moe', 'age': 40 }, { 'name': 'larry', 'age':
	 * 50 } ];
	 * 
	 * var shallow = _.clone(stooges); shallow[0] === stooges[0]; // => true
	 * 
	 * var deep = _.clone(stooges, true); deep[0] === stooges[0]; // => false
	 * 
	 ...
	 * 50 } ];
	 * 
	 * var deep = _.cloneDeep(stooges); deep[0] === stooges[0]; // => false
	 * 
	 ...
	 * @example
	 * 
	 * var food = { 'name': 'apple' }; _.defaults(food, { 'name': 'banana',
	 * 'type': 'fruit' }); // => { 'name': 'apple', 'type': 'fruit' }
	 */
	 ...
	 * @example
	 * 
	 * _.findKey({ 'a': 1, 'b': 2, 'c': 3, 'd': 4 }, function(num) { return num %
	 * 2 == 0; }); // => 'b' (property order is not guaranteed across
	 * environments)
	 */
	 ...
	 * @example
	 * 
	 * _.findLastKey({ 'a': 1, 'b': 2, 'c': 3, 'd': 4 }, function(num) { return
	 * num % 2 == 1; }); // => returns `c`, assuming `_.findKey` returns `a`
	 */
	 ...
	 * 
	 * _.forIn(new Dog('Dagny'), function(value, key) { console.log(key); }); // =>
	 * logs 'bark' and 'name' (property order is not guaranteed across
	 * environments)
	 */
	 ...
	 * Dog.prototype.bark = function() { console.log('Woof, woof!'); };
	 * 
	 * _.forInRight(new Dog('Dagny'), function(value, key) { console.log(key);
	 * }); // => logs 'name' and 'bark' assuming `_.forIn ` logs 'bark' and
	 * 'name'
	 */
    ...
    /**
	 * Checks if the specified object `property` exists and is a direct
	 * property, instead of an inherited property.
	 * 
	 * @static
	 * @memberOf _
	 * @category Objects
	 ...
	 * @example
	 * 
	 * _.invert({ 'first': 'moe', 'second': 'larry' }); // => { 'moe': 'first',
	 * 'larry': 'second' }
	 */
	 ...
	 * @example
	 * 
	 * var moe = { 'name': 'moe', 'age': 40 }; var copy = { 'name': 'moe',
	 * 'age': 40 };
	 * 
	 * moe == copy; // => false
	 * 
	 * _.isEqual(moe, copy); // => true
	 * 
	 ...
	 * @example
	 * 
	 * function Stooge(name, age) { this.name = name; this.age = age; }
	 * 
	 * _.isPlainObject(new Stooge('moe', 40)); // => false
	 * 
	 * _.isPlainObject([1, 2, 3]); // => false
	 * 
	 * _.isPlainObject({ 'name': 'moe', 'age': 40 }); // => true
	 */
	 ...
	 * @example
	 * 
	 * var names = { 'stooges': [ { 'name': 'moe' }, { 'name': 'larry' } ] };
	 * 
	 * var ages = { 'stooges': [ { 'age': 40 }, { 'age': 50 } ] };
	 * 
	 * _.merge(names, ages); // => { 'stooges': [{ 'name': 'moe', 'age': 40 }, {
	 * 'name': 'larry', 'age': 50 }] }
	 * 
	 ...
	 * @example
	 * 
	 * _.omit({ 'name': 'moe', 'age': 40 }, 'age'); // => { 'name': 'moe' }
	 * 
	 * _.omit({ 'name': 'moe', 'age': 40 }, function(value) { return typeof
	 * value == 'number'; }); // => { 'name': 'moe' }
	 */
	 ...
	 * @example
	 * 
	 * _.pairs({ 'moe': 30, 'larry': 40 }); // => [['moe', 30], ['larry', 40]]
	 * (property order is not guaranteed across environments)
	 */
	 ...
	 * @example
	 * 
	 * _.pick({ 'name': 'moe', '_userid': 'moe1' }, 'name'); // => { 'name':
	 * 'moe' }
	 * 
	 * _.pick({ 'name': 'moe', '_userid': 'moe1' }, function(value, key) {
	 * return key.charAt(0) != '_'; }); // => { 'name': 'moe' }
	 */
	 ...
	 * `accumulator` object. The callback is bound to `thisArg` and invoked with
	 ...
	 * 
	 * _.at(['moe', 'larry', 'curly'], 0, 2); // => ['moe', 'curly']
	 */
	 ...
	 * 
	 * _.contains({ 'name': 'moe', 'age': 40 }, 'moe'); // => true
	 * 
	 * _.contains('curly', 'ur'); // => true
	 */
	 ...
	 * 
	 * var stooges = [ { 'name': 'moe', 'age': 40 }, { 'name': 'larry', 'age':
	 * 50 } ]; // using "_.pluck" callback shorthand _.every(stooges, 'age'); // =>
	 * true // using "_.where" callback shorthand _.every(stooges, { 'age': 50
	 * }); // => false
	 */
	 ...
	 * 0; }); // => [2, 4, 6]
	 * 
	 * var food = [ { 'name': 'apple', 'organic': false, 'type': 'fruit' }, {
	 * 'name': 'carrot', 'organic': true, 'type': 'vegetable' } ]; // using
	 * "_.pluck" callback shorthand _.filter(food, 'organic'); // => [{ 'name':
	 * 'carrot', 'organic': true, 'type': 'vegetable' }] // using "_.where"
	 * callback shorthand _.filter(food, { 'type': 'fruit' }); // => [{ 'name':
	 * 'apple', 'organic': false, 'type': 'fruit' }]
	 */
	 ...
	 * @example
	 * 
	 * _.find([1, 2, 3, 4], function(num) { return num % 2 == 0; }); // => 2
	 * 
	 * var food = [ { 'name': 'apple', 'organic': false, 'type': 'fruit' }, {
	 * 'name': 'banana', 'organic': true, 'type': 'fruit' }, { 'name': 'beet',
	 * 'organic': false, 'type': 'vegetable' } ]; // using "_.where" callback
	 * shorthand _.find(food, { 'type': 'vegetable' }); // => { 'name': 'beet',
	 * 'organic': false, 'type': 'vegetable' } // using "_.pluck" callback
	 * shorthand _.find(food, 'organic'); // => { 'name': 'banana', 'organic':
	 * true, 'type': 'fruit' }
	 */
	 ...
	 * Math); // => { '4': [4.2], '6': [6.1, 6.4] } // using "_.pluck" callback
	 * shorthand _.groupBy(['one', 'two', 'three'], 'length'); // => { '3':
	 * ['one', 'two'], '5': ['three'] }
	 */
	 ...
	 * 
	 * _.indexBy(stooges, function(key) { this.fromCharCode(key.code); },
	 ...
	 * 
	 * var stooges = [ { 'name': 'moe', 'age': 40 }, { 'name': 'larry', 'age':
	 * 50 } ]; // using "_.pluck" callback shorthand _.map(stooges, 'name'); // =>
	 * ['moe', 'larry']
	 */
	 ...
	 * 50 } ];
	 * 
	 * _.max(stooges, function(stooge) { return stooge.age; }); // => { 'name':
	 * 'larry', 'age': 50 }; // using "_.pluck" callback shorthand
	 * _.max(stooges, 'age'); // => { 'name': 'larry', 'age': 50 };
	 */
	 ...
	 * 50 } ];
	 * 
	 * _.min(stooges, function(stooge) { return stooge.age; }); // => { 'name':
	 * 'moe', 'age': 40 }; // using "_.pluck" callback shorthand _.min(stooges,
	 * 'age'); // => { 'name': 'moe', 'age': 40 };
	 */
	 ...
	 * var food = [ { 'name': 'apple', 'organic': false, 'type': 'fruit' }, {
	 * 'name': 'carrot', 'organic': true, 'type': 'vegetable' } ]; // using
	 * "_.pluck" callback shorthand _.reject(food, 'organic'); // => [{ 'name':
	 * 'apple', 'organic': false, 'type': 'fruit' }] // using "_.where" callback
	 * shorthand _.reject(food, { 'type': 'fruit' }); // => [{ 'name': 'carrot',
	 * 'organic': true, 'type': 'vegetable' }]
	 */
	 ...
	 * var food = [ { 'name': 'apple', 'organic': false, 'type': 'fruit' }, {
	 * 'name': 'carrot', 'organic': true, 'type': 'vegetable' } ]; // using
	 * "_.pluck" callback shorthand _.some(food, 'organic'); // => true // using
	 * "_.where" callback shorthand _.some(food, { 'type': 'meat' }); // =>
	 * false
	 */
	 ...
	 * [3, 1, 2] // using "_.pluck" callback shorthand _.sortBy(['banana',
	 * 'strawberry', 'apple'], 'length'); // => ['apple', 'banana',
	 * 'strawberry']
	 */
	 ...
	 *            collection The collection to iterate over.
	 * @param {Object}
	 *            properties The object of property values to filter by.
	 * @returns {Array} Returns a new array of elements that have the given
	 *          properties.
	 * @example
	 * 
	 * var stooges = [ { 'name': 'curly', 'age': 30, 'quotes': ['Oh, a wise guy,
	 * eh?', 'Poifect!'] }, { 'name': 'moe', 'age': 40, 'quotes': ['Spread
	 * out!', 'You knucklehead!'] } ];
	 * 
	 * _.where(stooges, { 'age': 40 }); // => [{ 'name': 'moe', 'age': 40,
	 * 'quotes': ['Spread out!', 'You knucklehead!'] }]
	 * 
	 * _.where(stooges, { 'quotes': ['Poifect!'] }); // => [{ 'name': 'curly',
	 * 'age': 30, 'quotes': ['Oh, a wise guy, eh?', 'Poifect!'] }]
	 */
    var where = filter;
	 ...
	 * @example
	 * 
	 * _.findIndex(['apple', 'banana', 'beet'], function(food) { return
	 * /^b/.test(food); }); // => 1
	 */
	 ...
	 * @example
	 * 
	 * _.findLastIndex(['apple', 'banana', 'beet'], function(food) { return
	 * /^b/.test(food); }); // => 2
	 */
	 ...
	 * 
	 * var food = [ { 'name': 'banana', 'organic': true }, { 'name': 'beet',
	 * 'organic': false }, ]; // using "_.pluck" callback shorthand
	 * _.first(food, 'organic'); // => [{ 'name': 'banana', 'organic': true }]
	 * 
	 * var food = [ { 'name': 'apple', 'type': 'fruit' }, { 'name': 'banana',
	 * 'type': 'fruit' }, { 'name': 'beet', 'type': 'vegetable' } ]; // using
	 * "_.where" callback shorthand _.first(food, { 'type': 'fruit' }); // => [{
	 ...
	 * 
	 * var stooges = [ { 'name': 'curly', 'quotes': ['Oh, a wise guy, eh?',
	 * 'Poifect!'] }, { 'name': 'moe', 'quotes': ['Spread out!', 'You
	 * knucklehead!'] } ]; // using "_.pluck" callback shorthand
	 * _.flatten(stooges, 'quotes'); // => ['Oh, a wise guy, eh?', 'Poifect!',
	 ...
	 * 
	 * var food = [ { 'name': 'beet', 'organic': false }, { 'name': 'carrot',
	 * 'organic': true } ]; // using "_.pluck" callback shorthand
	 * _.initial(food, 'organic'); // => [{ 'name': 'beet', 'organic': false }]
	 * 
	 * var food = [ { 'name': 'banana', 'type': 'fruit' }, { 'name': 'beet',
	 * 'type': 'vegetable' }, { 'name': 'carrot', 'type': 'vegetable' } ]; //
	 * using "_.where" callback shorthand _.initial(food, { 'type': 'vegetable'
	 ...
	 * 
	 * var food = [ { 'name': 'beet', 'organic': false }, { 'name': 'carrot',
	 * 'organic': true } ]; // using "_.pluck" callback shorthand _.last(food,
	 * 'organic'); // => [{ 'name': 'carrot', 'organic': true }]
	 * 
	 * var food = [ { 'name': 'banana', 'type': 'fruit' }, { 'name': 'beet',
	 * 'type': 'vegetable' }, { 'name': 'carrot', 'type': 'vegetable' } ]; //
	 * using "_.where" callback shorthand _.last(food, { 'type': 'vegetable' }); // => [{
	 * 'name': 'beet', 'type': 'vegetable' }, { 'name': 'carrot', 'type':
	 * 'vegetable' }]
	 */
    ...
    function range(start, end, step) {
      ...
      // use `Array(length)` so engines, like Chakra and V8, avoid slower
		// modes
	 ...
	 * 
	 * var food = [ { 'name': 'banana', 'organic': true }, { 'name': 'beet',
	 * 'organic': false }, ]; // using "_.pluck" callback shorthand _.rest(food,
	 * 'organic'); // => [{ 'name': 'beet', 'organic': false }]
	 * 
	 * var food = [ { 'name': 'apple', 'type': 'fruit' }, { 'name': 'banana',
	 * 'type': 'fruit' }, { 'name': 'beet', 'type': 'vegetable' } ]; // using
	 * "_.where" callback shorthand _.rest(food, { 'type': 'fruit' }); // => [{
	 ...
	 * @example
	 * 
	 * _.sortedIndex([20, 30, 50], 40); // => 2 // using "_.pluck" callback
	 * shorthand _.sortedIndex([{ 'x': 20 }, { 'x': 30 }, { 'x': 50 }], { 'x':
	 * 40 }, 'x'); // => 2
	 * 
	 ...
	 * Math); // => [1, 2.5, 3] // using "_.pluck" callback shorthand _.uniq([{
	 ...
	 * @example
	 * 
	 * _.zip(['moe', 'larry'], [30, 40], [true, false]); // => [['moe', 30,
	 * true], ['larry', 40, false]]
	 */
    function zip() {
	 ...
	 * @example
	 * 
	 * _.zipObject(['moe', 'larry'], [30, 40]); // => { 'moe': 30, 'larry': 40 }
	 */
	 ...
	 * 
	 * func = _.bind(func, { 'name': 'moe' }, 'hi'); func(); // => 'hi moe'
	 */
	 ...
	 * @example
	 * 
	 * var object = { 'name': 'moe', 'greet': function(greeting) { return
	 * greeting + ' ' + this.name; } };
	 * 
	 * var func = _.bindKey(object, 'greet', 'hi'); func(); // => 'hi moe'
	 * 
	 * object.greet = function(greeting) { return greeting + ', ' + this.name +
	 * '!'; };
	 * 
	 * func(); // => 'hi, moe!'
	 */
	 ...
	 * 
	 * var welcome = _.compose(greet, format); welcome('curly'); // => 'Hiya
	 * Jerome!'
	 */
    function compose() {
      var funcs = arguments,
	 ...
	 * 
	 * var stooges = [ { 'name': 'moe', 'age': 40 }, { 'name': 'larry', 'age':
	 * 50 } ]; // wrap to create custom callback shorthands _.createCallback =
	 ...
	 * 
	 * _.filter(stooges, 'age__gt45'); // => [{ 'name': 'larry', 'age': 50 }]
	 */
	 ...
	 *          jQuery(window).on('resize', lazyLayout); // execute `sendMail`
	 *          when the click event is fired, debouncing subsequent calls
	 *          jQuery('#postbox').on('click', _.debounce(sendMail, 300, {
	 *          'leading': true, 'trailing': false }); // ensure `batchLog` is
	 *          executed once after 1 second of debounced calls var source = new
	 *          EventSource('/stream'); source.addEventListener('message',
	 *          _.debounce(batchLog, 250, { 'maxWait': 1000 }, false);
	 */
	 ...
	 * @example
	 * 
	 * _.defer(function() { console.log('deferred'); }); // returns from the
	 * function before 'deferred' is logged
	 */
    function defer(func) {
      if (!isFunction(func)) {
        throw new TypeError;
	 ...
	 * @example
	 * 
	 * var log = _.bind(console.log, console); _.delay(log, 1000, 'logged
	 * later'); // => 'logged later' (Appears after one second.)
	 */
	 ...
	 * 1) + fibonacci(n - 2); });
	 * 
	 * var data = { 'moe': { 'name': 'moe', 'age': 40 }, 'curly': { 'name':
	 * 'curly', 'age': 60 } }; // modifying the result cache var stooge =
	 * _.memoize(function(name) { return data[name]; }, _.identity);
	 * stooge('curly'); // => { 'name': 'curly', 'age': 60 }
	 * 
	 * stooge.cache.curly.name = 'jerome'; stooge('curly'); // => { 'name':
	 * 'jerome', 'age': 60 }
	 */
	 ...
	 * var hi = _.partial(greet, 'hi'); hi('moe'); // => 'hi moe'
	 */
	 ...
	 * @example // avoid excessively updating the position while scrolling var
	 *          throttled = _.throttle(updatePosition, 100);
	 *          jQuery(window).on('scroll', throttled); // execute `renewToken`
	 *          when the click event is fired, but not more than once every 5
	 *          minutes jQuery('.interactive').on('click',
	 ...
	 * @example
	 * 
	 * var hello = function(name) { return 'hello ' + name; }; hello =
	 * _.wrap(hello, function(func) { return 'before, ' + func('moe') + ',
	 ...
	 * @category Utilities
	 * @param {Object}
	 *            object The object of function properties to add to `lodash`.
	 * @param {Object}
	 *            object The object of function properties to add to `lodash`.
	 * @example
	 * 
	 * _.mixin({ 'capitalize': function(string) { return
	 * string.charAt(0).toUpperCase() + string.slice(1).toLowerCase(); } });
	 * 
	 * _.capitalize('moe'); // => 'Moe'
	 * 
	 * _('moe').capitalize(); // => 'Moe'
	 */
    ...
    var parseInt = nativeParseInt(whitespace + '08') == 8 ? nativeParseInt : function(value, radix) {
      // Firefox and Opera still follow the ES3 specified implementation of
		// `parseInt`
	 ...
	 * @example // using the "interpolate" delimiter to create a compiled
	 *          template var compiled = _.template('hello <%= name %>');
	 *          compiled({ 'name': 'moe' }); // => 'hello moe' // using the
	 *          "escape" delimiter to escape HTML in data property values
	 ...
	 *          <li><%- name %></li><% }); %>'; _.template(list, { 'people':
	 *          ['moe', 'larry'] }); // => '
	 *          <li>moe</li>
	 *          <li>larry</li>' // using the ES6 delimiter as an alternative
	 *          to the default "interpolate" delimiter _.template('hello ${ name
	 *          }', { 'name': 'curly' }); // => 'hello curly' // using the
	 *          internal `print` function in "evaluate" delimiters _.template('<%
	 *          print("hello " + name); %>!', { 'name': 'larry' }); // => 'hello
	 *          larry!' // using a custom template delimiters _.templateSettings = {
	 *          'interpolate': /{{([\s\S]+?)}}/g };
	 * 
	 * _.template('hello {{ name }}!', { 'name': 'mustache' }); // => 'hello
	 * mustache!' // using the `imports` option to import jQuery var list = '<%
	 * $.each(people, function(name) { %>
	 * <li><%- name %></li><% }); %>'; _.template(list, { 'people': ['moe',
	 * 'larry'] }, { 'imports': { '$': jQuery } }); // => '
	 * <li>moe</li>
	 * <li>larry</li>' // using the `sourceURL` option to specify a custom
	 * sourceURL for the template var compiled = _.template('hello <%= name %>',
	 * null, { 'sourceURL': '/basic/greeting.jst' }); compiled(data); // => find
	 * the source of "greeting.jst" under the Sources tab or Resources panel of
	 * the web inspector // using the `variable` option to ensure a
	 * with-statement isn't used in the compiled template var compiled =
	 * _.template('hi <%= data.name %>!', null, { 'variable': 'data' });
	 * compiled.source; // => function(data) { var __t, __p = '', __e =
	 * _.escape; __p += 'hi ' + ((__t = ( data.name )) == null ? '' : __t) +
	 * '!'; return __p; } // using the `source` property to inline compiled
	 * templates for meaningful // line numbers in error messages and a stack
	 * trace fs.writeFileSync(path.join(cwd, 'jst.js'), '\ var JST = {\ "main": ' +
	 ...
	 * 
	 * var stooges = [ { 'name': 'moe', 'age': 40 }, { 'name': 'larry', 'age':
	 * 50 }, { 'name': 'curly', 'age': 60 } ];
	 * 
	 * var youngest = _.chain(stooges) .sortBy('age') .map(function(stooge) {
	 * return stooge.name + ' is ' + stooge.age; }) .first() .value(); // =>
	 * 'moe is 40'
	 */
	 ...
	 * @example
	 * 
	 * _([1, 2, 3, 4]) .filter(function(num) { return num % 2 == 0; })
	 * .tap(function(array) { console.log(array); }) .map(function(num) { return
	 * num * num; }) .value(); // => // [2, 4] (logged) // => [4, 16]
	 */
	 ...
	 * 
	 * var stooges = [ { 'name': 'moe', 'age': 40 }, { 'name': 'larry', 'age':
	 * 50 } ]; // without explicit chaining _(stooges).first(); // => { 'name':
	 * 'moe', 'age': 40 } // with explicit chaining _(stooges).chain() .first()
	 * .pick('age') .value() // => { 'age': 40 }
	 */
...
(function(root, factory) {
    ...
    VElement.prototype = {
	    ...
	    // Setting `dominant-baseline` on the `<text>` element doesn't work in
		// IE9.
		...
		// IE9 throws an exception in odd cases. (`Unexpected call to method or
		// property access`)
...
joint.dia.Graph = Backbone.Model.extend({
    ...
    fromJSON: function(json) {
        ...
        // Cells are the only attribute that is being set differently, using
		// `cells.add()`.
...
joint.dia.CellView = Backbone.View.extend({
    ...
    // `selector` is being collected through the recursive call. No value for
	// `selector` is expected when using this method.
...
joint.dia.ElementView = joint.dia.CellView.extend({
	...
	var velbbox = vel.bbox(false, this.paper.viewport);
        // `y-alignment` when set to `middle` causes centering of the subelement
		// around its new y coordinate.
        ...
        // `x-alignment` when set to `middle` causes centering of the subelement
		// around its new x coordinate.
    ...
    pointermove: function(evt, x, y) {
	    ...
	    var grid = this.paper.options.gridSize;
	        ...
	        // Make sure the new element's position always snaps to the current
			// grid after
...
joint.dia.LinkView = joint.dia.CellView.extend({
    ...
    update: function() {
        ...
        if (this.model.get('manhattan')) {
            // If manhattan routing is enabled, find new vertices so that the
			// link is orthogonally routed.
    ...
    updateLabelPositions: function() {
        ...
        _.each(labels, function(label, idx) {
            ...
            position = (position > connectionLength) ? connectionLength : position; // sanity
																					// check
    ...
    updateToolsPosition: function() {
        ...
        // If the link is too short, make the tools half the size and the offset
		// twice as low.
    ...
    updateArrowheadMarkers: function() {
        ...
        // Make the markers "point" to their sticky points being auto-oriented
		// towards `targetPosition`/`sourcePosition`.
    ...
    _createWatcher: function(endType) {
        ...
        function watchEnd(link, end) {
            ...
            // Pick updateMethod this._sourceBboxUpdate or
			// this._targetBboxUpdate.
    ...
    // It's important to keep both methods (sourceBboxUpdate and
	// targetBboxUpdate) as unique methods
    // because of loop links. We have to be able to determine, which method we
	// want to stop listen to.
    ...
    addVertex: function(vertex) {
        ...
        // In manhattan routing, if there are no vertices, the path length
		// changes significantly
        // with the first vertex added. Shall we check vertices.length === 0? at
		// beginning of addVertex()
        // in order to avoid the temporary path construction and other
		// operations?
    ...
    getPathData: function(vertices) {
		...
		// its scale around x-axis.
        // Note that scale is the only transform that makes sense to be set in
		// `.marker-source` attributes object
        // as all other transforms (translate/rotate) will be replaced by the
        ...
        if (this.model.get('smooth')) {
            ...
            if (vertices && vertices.length) {
                ...
                // the two control points are both defined with X as mid way
				// between the source and target points.
                // sourceControlPoint Y is equal to sourcePoint Y and
				// targetControlPointY being equal to targetPointY.
                // handle situation were sourcePointX is greater or less then
				// targetPointX.
	...
	// have a manhattan link routing from
    ...
    findManhattanRoute: function(vertices) {
        ...
        // Return the direction that one would have to take traveling from `p1`
		// to `p2`.
        ...
        function bestDirection(p1, p2, preferredDirection) {
            ...
            // This branching determines possible directions that one can take
			// to travel
            // from `p1` to `p2`.
            if (p1.x < p2.x) {
            ...
            // Should the direction be the exact opposite of the preferred
			// direction,
        ...
        // Find a vertex in between the vertices `p1` and `p2` so that the route
		// between those vertices
        // is orthogonal. Prefer going the direction determined by
		// `preferredDirection`.
        ...
        for (var i = 0; i < vertices.length - 1; i++) {
            ...
            if (i > 0) {
                ...
                // Determine a direction between the last vertex and the new
				// one.
                // Therefore, each vertex contains the `d` property describing
            ...
            // Make sure that we don't create a vertex that would go the
			// opposite direction then that of the
            // previous one. Othwerwise, a 'spike' segment would be created
			// which is not desirable.
            // Find a dummy vertex to keep the link orthogonal. Preferably, take
			// the same direction
            // as the previous one.
            var d = lastManhattanVertex && lastManhattanVertex.d;
            manhattanVertex = findMiddleVertex(vertex, nextVertex, d);

            // Do not add a new vertex that is the same as one of the vertices
			// already added.
	...
	this._dx = x;
        ...
        default:
            ...
            if (targetParentEvent) {
                ...
                // `remove` event is built-in. Other custom events are triggered
				// on the paper.
...
joint.dia.Paper = Backbone.View.extend({
    ...
    resetCells: function(cellsCollection) {
        ...
        // Sort the cells in the DOM manually as we might have changed the order
		// they
    ...
    pointerdown: function(evt) {
        ...
        if (view) {
			...
			this.lastX = localPoint.x;
			this.lastY = localPoint.y;
    ...
    pointerup: function(evt) {
        ...
        if (this.sourceView) {
			...
			if(this.lastX == localPoint.x && this.lastY == localPoint.y)
			{
...
joint.shapes.basic.TextBlock = joint.shapes.basic.Rect.extend({
    ...
    setTextContent: function(cell, content) {
        ...
        // This could be overriden in order to break the text lines to fit to a
